...
ErrCode SpecialistCtl::oneCSVItemCallback(const QStringList &items, void *param)
{
...
void SpecialistCtl::onLoad()
{
    ...
    if (!FileCtl::checkPrebuiltDataFileHash(fname)){
        ret = Utils::parseCSVFile(FileCtl::getPrebuiltDataFilePath(fname),
                                  &SpecialistCtl::oneCSVItemCallback);
        if (ret == ErrNone){
            mLoaded = true;
    ...
    QList<DbModel*> items =DbCtl::getDb()->getSpecialistModelHandler()->getAll(&Specialist::builder);
